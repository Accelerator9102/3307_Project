...
class APIHelper {
	...
	public:
		...
		/// @param The token used to ensure duplicate questions are not received.
		/// @returns A random question from the API in Json format.
		Json::Value getQuestion(std::string token);
		
		/// @brief Method to retrieve a token from the API.
		/// @returns A string representing the generated token from the API.
		std::string getToken();
		
		// Not used for prototype
		/// @brief Method to reset a token so that seen questions may be received again.
		/// @param The token to be reset.
		/// @returns The code from the API representing success or failure.
	...
	private:
		...
		/// @param A pointer to the contents of the URL.
		/// @param Size.
		/// @param Number of members.
		/// @param A pointer to the string buffer to store data into.
		/// @returns The real size of the file.
		static size_t callback(void *contents, size_t size, size_t nmemb, void *userp);
		
		/// @brief Method used to directly make calls to the API.
		/// @param The URL to pass into CURL for retrieval.
		/// @returns The Json object retrieved from parsing through the buffer string or -1 if it failed.
