...
Question::Question(string token) {
	...
	// Due to one of our IO pins on the Raspberry PI not working, we've omitted one incorrect option.
	int answerOffset = rand() % 3;
	options = vector<string>(3);
	options[(0 + answerOffset) % 3] = questionData["results"][0]["correct_answer"].asString();
	options[(1 + answerOffset) % 3] = questionData["results"][0]["incorrect_answers"][0].asString();
	options[(2 + answerOffset) % 3] = questionData["results"][0]["incorrect_answers"][1].asString();
	
	//options[(3 + answerOffset) % 3] = questionData["results"][0]["incorrect_answers"][2].asString();
	correctAnswer = (0 + answerOffset) % 3;
}
